package com.example.loginapp.payment
...
class PagarMeManager(private val context: Context) {
    ...
    suspend fun processPayment(
        ...
        return try {
            ...
            val billing = com.example.loginapp.models.payment.BillingAddress(
            ...
            val creditCardData = com.example.loginapp.models.payment.CreditCardData(
                card = cardData,
                billingAddress = billing
            )
            
            // Criar método de pagamento (estrutura v5)
            val creditCardPaymentMethod = com.example.loginapp.models.payment.CreditCardPaymentMethod(
            ...
            val paymentRequest = com.example.loginapp.models.payment.PaymentRequest(
                items = listOf(item),
    ...
    suspend fun processPixPayment(
        customerInfo: com.example.loginapp.models.payment.CustomerInfo,
        amount: Double,
        description: String,
        orderId: String
    ): PixPaymentResult {
        return try {
            ...
            // Criar item do pedido
            val item = com.example.loginapp.models.payment.PaymentItem(
                amount = amountInCents,
                description = description,
                quantity = 1,
                code = orderId
            ...
            // Criar dados PIX
            val pixData = com.example.loginapp.models.payment.PixData(
                expiresIn = 3600, // 1 hora
                additionalInformation = listOf(
                    com.example.loginapp.models.payment.PixAdditionalInfo(
                        name = "Pedido",
                        value = orderId
                    )
                )
            ...
            val pixPaymentMethod = com.example.loginapp.models.payment.PixPaymentMethod(
                paymentMethod = "pix",
                pix = pixData
            )
            ...
            val pixRequest = com.example.loginapp.models.payment.PixPaymentRequest(
                items = listOf(item),
                customer = customerInfo,
