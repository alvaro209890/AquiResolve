...
- **Quantidade por item (2026-07-09):** cada item do combo tem `quantity` (1..20, default 1 — combos antigos valem 1). Painel: stepper `− N× +` no chip do serviço; validação = **2+ unidades somando quantidades**; `fullPrice`/coerência multiplicam pela quantidade. App: detalhe mostra "3× Serviço" com preço da linha; "Adicionar ao carrinho" cria **um item por unidade** (`repeat(quantity)`) — o desconto do carrinho segue automático. Regras Firestore inalteradas. **Respeitar quantidade no app exige APK novo.** Detalhes: `docs/COMBOS_QUANTIDADE_2026-07-09.md`.
...
| **Barras do sistema cobrindo topo/rodapé do app** (Android 15+) e conteúdo sob a status bar (Android antigo) | Tema com `windowTranslucentStatus` + hacks `SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN` em 10 telas + `InsetsHelper` bugado (2º listener na mesma view anulava o topo) + edge-to-edge FORÇADO no targetSdk 35 sem tratamento de insets | **CORRIGIDO 2026-07-09**: tema com status bar sólida (sem translucent, Android <15 volta a ser gerenciado pelo sistema) + tratador GLOBAL `EdgeToEdgeInsets` (Application-level, Android 15+, insets+IME+faixas coloridas, CONSOME insets) + markers `InsetsSelfManaged`/`StatusBarStripColor`. NUNCA reintroduzir translucent/fullscreen nem listener de insets no decorView. Exige novo APK. Detalhes: `docs/CORRECAO_LAYOUT_INSETS_2026-07-09.md` |
